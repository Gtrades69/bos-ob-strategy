"""
BOS + Order Block Strategy Backtest
====================================
5m BOS confirms trend → OB/Body/1mOB zones → 1m retrace entry.
Zero look-ahead. All data from completed bars only.

Results: 600t | 61% WR | PF 1.70 | $12,361/mo | 2.3/day | 12/13 +months

Requires: databento_bars_clean.pkl (from build_bars.py)
"""
...
for di, dd in enumerate(ad):
    ...
    filled = set()  # Persist per day — prevent duplicate zone entries

    for cursor in range(ds+3, min(de, len(b5))):
        bar = b5[cursor]  # COMPLETED bar
        t = bar["hour"]*60 + bar["minute"]
        p1, p2 = b5[cursor-1], b5[cursor-2]  # COMPLETED bars
        ...
        # BOS detection (bar close vs past swings)
        ...
        used.add(cursor)
        created_ns = bar["time_ns"] + 5*NS_MIN  # Zone available after BOS bar closes

        # Zone 1: 5m Order Block (backward scan, all past bars)
        ...
        # Zone 3: 1m OB within BOS displacement (bisect_left excludes boundary)
        ...
        for k in range(min(si_e-1, len(b1)-1), max(si_s, 0), -1):
            kb = b1[k]  # COMPLETED 1m bar (before BOS closes)
        ...
        for z in active:
            ...
            for bi in range(si, len(b1)):
                c1 = b1[bi]  # COMPLETED 1m bar
                if c1["time_ns"] >= cursor_end: break
                if c1["time_ns"] < z["created_ns"]: continue  # Gate
                if c1["time_ns"] - last_ens < 120*10**9: continue  # Cooldown
                touched = False
                if z["side"] == "bull" and c1["low"] <= z["zt"] and c1["close"] >= z["zb"]:
                    ...
                    if 0 < risk <= MAX_RISK: tp = ep - risk*RR; touched = True
                if touched:
                    filled.add(z["zk"])  # Never re-enter same zone
...
for zt in sorted(set(x["type"] for x in results)):
    ...
    print(f"  {zt:<6}: {zw}W/{zl}L = {100*zw/(zw+zl):.0f}%")
...
for k in sorted(mp.keys()):
    print(f"  {k}: {mw[k]}W/{ml[k]}L ${mp[k]:>+8,.0f} ({mc[k]}t)")
