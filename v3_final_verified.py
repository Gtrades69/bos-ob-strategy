"""
BOS+OB V3 FINAL — VERIFIED CLEAN
==================================
71% WR | PF 2.49 | $19,393/mo | 13/13 +months | 2.3 trades/day
3ct NQ | 1yr Databento data | Zero look-ahead confirmed

Strategy:
  1. Track 5m swing highs/lows (min 5pt depth)
  2. BOS = candle BODY CLOSES beyond last swing in confirmed trend (HH+HL or LL+LH)
  3. BOS candle must be displacement (body >= 35% of range)
  4. Create zones:
     - BODY = BOS candle body (69% WR)
     - 1mOB = last opposing 1m candle in BOS displacement (75% WR)
  5. Wait for 1m pullback to touch zone → entry at 1m close
  6. Stop at displacement extreme
  7. Target = 1.1x risk
  8. Zone expires after 45 minutes
  9. Min risk 5pts (filter noise)
  10. Session: London+NY (02:00-14:30 CT), skip hours 7/12/14

Config: RR=1.1, bp35, London+NY, skip 7/12/14, zone_age 45min, min_risk 5pt
Alt:    RR=1.3 → 65% WR, $19,743/mo (higher PnL, lower WR)

Audit: Every data access uses ONLY completed past bars. Zero look-ahead.
  - Swings: confirmed by next bar
  - BOS: completed bar close vs past swing
  - Zones: available AFTER BOS bar closes (created_ns gate)
  - 1mOB: bisect_left excludes boundary (no forming bars)
  - Entry: 1m close + slip after zone gate
  - Sim: starts AFTER entry bar (bisect_right)
  - Stop-first bias: conservative ~3-4% below real WR
"""
...
RR = 1.1
MIN_BODY_PCT = 0.35
SESSION_START = 120    # 02:00 CT (London)
SESSION_END = 870      # 14:30 CT
SKIP_HOURS = {7, 12, 14}
ZONE_MAX_AGE = 45      # minutes
MIN_RISK = 5           # pts
MAX_RISK = 30
MAX_ED = 5
COOLDOWN_S = 120
...
for di, dd in enumerate(ad):
    ...
    for cursor in range(ds+3, min(de, len(b5))):
        ...
        used.add(cursor)
        created_ns = bar["time_ns"] + 5*NS_MIN  # Gate: zone available after bar closes
        ...
        si_e = bisect.bisect_left(b1_ns, disp_end)  # Excludes boundary
...
for k in sorted(mp.keys()):
    print(f"  {k}: {mw[k]}W/{ml[k]}L ${mp[k]:>+9,.0f} ({mc[k]}t)")
